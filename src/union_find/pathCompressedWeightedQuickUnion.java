...
public class pathCompressedWeightedQuickUnion {
    ...
    public int root(int i) {
        while (i != id[i]) { // chase parent pointers until the pointer value is equal to the point (indicating top of root)
            id[i] = id[id[i]]; // sets the parent of the next level up equal to the grandparent, simplifying the jump from child -> parent -> grandparent to child -> grandparent
            i = id[i];
        }
        return i;
    }
    ...
    public static void main(String[] args) {
        ...
        pathCompressedWeightedQuickUnion uf = new pathCompressedWeightedQuickUnion(N);
        for (int i = 0; i < 11; i++) {
            int p = pArray[i];
            int q = qArray[i];
            if (!uf.connected(p, q)) {
